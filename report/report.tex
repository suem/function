...
Both versions follow the same structure. We will first discuss the universal version.\\
...
All partitions that satisfy $\Phi_1$ are set to zero and all partitions that neither satisfy $\Phi_1$ nor $\Phi_2$ are discarded.
That way we end up with a decision tree that is only defined for those partitions which satisfy $\forall(\Phi_1 U \Phi_2)$.\\

The abstract tansformer for the existential case follows the same structure. 
