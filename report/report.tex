...
This decision tree represents the value of the ranking function for the program label of this node. 
...
For assignments and boolean conditions we use the corresponding B-ASSIGN and FILTER operators.\\

What is left is how to compute the abstract CTL semantics for the different path dependent CTL properties. 
All are computed through fixed-point iteration on the control flow graph. In general, the abstract CTL semantics $\tau^\sharp_\Phi$
are a fixed point of the abstract transformer $\phi^\sharp_\Phi$ w.r.t the computational order $\sqsubseteq_{\mathcal{T}}$. 

\begin{align*}
    \tau^\sharp_\Phi &= lfp^{\sqsubseteq_{\mathcal{T}}} \phi^\sharp_\Phi  \\
    \phi^\sharp_\Phi &\in (\mathcal{L} \rightarrow \mathcal{T}_{NIL}) \rightarrow (\mathcal{L} \rightarrow \mathcal{T}_{NIL})
\end{align*}

The initial value for the fixed-point computation and the definition of $\phi^\sharp_\Phi$
depend on the semantic of the corresponding CTL property $\Phi$. Note that $\phi^\sharp_\Phi$ must approximate 
the abstract transformer $\phi_\Phi$ of the concrete CTL semantics.\\

\textbf{Until}\\
The abstract transformers for the universal and existential `until' properties are given in definition~\ref{def:abstract_until_semantics}. 
Both versions follow the same structure. The fixed-point iterations starts fro the totally undefined decision tree $\bot$. 
...
\begin{definition}\label{def:abstract_until_semantics}
    ...
    \setlength{\jot}{15pt}
    \begin{align*}
        \tau^\sharp_{\forall(\Phi_1 U \Phi_2)} \ &\eqdef \ lfp^{\sqsubseteq_{\mathcal{T}}}_{\bot} \phi^\sharp_{\forall(\Phi_1 U \Phi_2)}\\
        t_{\curlyvee}(l)  \ &\eqdef \ \bigcurlyvee\limits_{(l, stmt, l') \ \in \ out(l) } {\llbracket stmt \rrbracket}_{o} ( m(l')) \\
        \phi^\sharp_{\forall(\Phi_1 U \Phi_2)}(m)l \ &\eqdef \ UNTIL\llbracket \tau^\sharp_{\Phi_1}, \tau^\sharp_{\Phi_2} \rrbracket (t_{\curlyvee}(l))  \\\\
        %
        %
        \tau^\sharp_{\exists(\Phi_1 U \Phi_2)} \ &\eqdef \ lfp^{\sqsubseteq_{\mathcal{T}}}_{\bot} \phi^\sharp_{\exists(\Phi_1 U \Phi_2)}\\
        t_{\sqcup}(l) \ &\eqdef \ \bigsqcup\limits_{(l, stmt, l') \ \in \ out(l) } {\llbracket stmt \rrbracket}_{u} (m(l')) \\
        \phi^\sharp_{\exists(\Phi_1 U \Phi_2)}(m)l \ &\eqdef \ UNTIL\llbracket \tau^\sharp_{\Phi_1}, \tau^\sharp_{\Phi_2} \rrbracket ( t_{\sqcup}(l))
    \end{align*}
\end{definition}
...
are combined using the \textit{approximation join} $\curlyvee$. The \textit{approximation join} discards all partitions of
decision trees that are not defined for all successor nodes. By doing so, we approximate the semantic of the universal path  
quantifier $\forall$.\\ 
...
Decision trees produced by ${\llbracket \cdot \rrbracket}_{o}$ might contain defined partitions for states that are unfeasible 
...
applied to he $UNTIL\llbracket \tau^\sharp_{\Phi_1}, \tau^\sharp_{\Phi_2} \rrbracket $ operator (see definition XY TODO). 
This operator approximates the semantics of the `until' operator. 
...
\begin{algorithm}                      
    ...
    \begin{algorithmic}
        \Function{reset\_until}{$t, t_{\text{reset}}$} 
            ...
            \EndIf 
        \EndFunction
        \Function{until}{$t, t_{\Phi_1}, t_{\Phi_2}$} 
            \Comment{$t, t_{\Phi_1}, t_{\Phi_2} \in \mathcal{T}_{NIL}$}
            \State $t_{\text{filtered}} \gets \Call{filter\_until}{\Call{tree\_unification}{t, t_{\Phi_1} \sqcup t_{\Phi_2}}}$
            \State \Return $\Call{reset\_until}{\Call{tree\_unification}{t_{\text{filtered}}, t_{\Phi_2}}}$
        \EndFunction
\end{algorithmic}
\end{algorithm}
...
\textbf{Global}\\
The abstract transformers for the universal and existential `global' properties are given in definition~\ref{def:abstract_global_semantics}. 
