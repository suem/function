...
\section{Concrete Semantics for CTL}
...
\section{Decision Tree Abstract Domain}
...
An element $t \in \mathcal{T}$ is either a \textit{leaf node} $LEAF \colon f$ consisting of a function $f \in \mathcal{F}$, or a \textit{decision node}
$NODE\{c\}: l; r$ consists of a linear constraint $c \in \mathcal{C}$ (denoted $t.c$) and a left and a right sub tree $l, r \in \mathcal{T}$ 
(denoted $t.l$ and $t.r$).
\[
...
This section presents the abstract interpretation framework for analysing CTL properties. 
First we recall the concept of ranking functions and define how ranking functions relate to 
the satisfiability of CTL properties. Then we describe the concrete semantics of ranking functions for CTL properties.
