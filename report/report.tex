...
all remaining states $a$, $c$ and $d$ have at least one edge to a node thats part of the function. In the universal case we get an additional
...
Note that the \textit{filter}, \text{backward assign} and $\text{reset}$ operators were all implemented using over-approximation. 
...
Let us consider the program \textsf{test\_until}. Assuming that initially $x \geq y$ holds, one can show that the program satisfies 
the property $\forall(x \geq y \ U \ x = y)$. FuncTion is able to infer the following 
counting function for the entry point of the program:
...
Consider program \textsf{test\_global}. One can see that the property $\forall\lozenge\forall\square(y > 0)$ 
is satisfied if $x < 10$ holds initially. FuncTion can prove this by deriving the following counting function:
...
The program \textsf{test\_existential\_1} initially sets $x$ to zero and assigns a random number to $i$.
This value is then added to $x$ as long as $x \geq 0$. One can easily see that there exists 
a choice for $i$ such that $x$ will eventually an arbitrary positive value. Therefore \textsf{test\_existential}
satisfies the existential CTL property $\exists\lozenge(x > 2048)$. 
FuncTion is able to infer the following counting function for the entry point of the function:
...
where the property is satisfied in at most 4 steps.
...
The program \textsf{test\_existential\_2} increments $x$ an arbitrary amount of time. If $x$ reaches $200$ then $r$
...
The program \textsf{acqrel} was taken from a set of example 
programs\footnote{\url{https://github.com/ultimate-pa/ultimate/tree/dev/trunk/examples/LTL/koskinen}} 
written by Koskinen for his PhD thesis~\cite{KoskinenPhd}. We assume that initially $a = r = 0$.
...
We can solve this problem by slightly modifying \textsf{acqrel}. In program \textsf{acqrel-mod}, the choice for the number of inner loop executions is 
moved to the start of the program. Since the information about $n$ is no longer relevant at the head of the outer loop, FuncTion is able to prove the 
property for \textsf{acqrel-mod}.\\
