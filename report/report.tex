...
$\tau_{\exists\square\Phi}$ and $\tau_{\forall\square\Phi}$. Both iterations start with some initial degraded ranking function $\tau_\Phi$.
...
that satisfies the property to construct a valid degraded ranking function according to Theorem~\ref{thr:ctl_semantics}.
...
that satisfy the boolean expression remain in the tree. In addition to the over-approximating version there is also the under-approximating version
...
has a small error that can lead to unsoundness. This is discussed in the excursion below titled `\text{RESET} and over-approximation'.
...
\begin{tcolorbox}
    \subsubsection*{\text{RESET} and over-approximation}
