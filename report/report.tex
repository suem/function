...
The \textit{CTL semantics} presented here are an extension of this work. We will define the \textit{CTL semantics} inductively 
for each CTL operator such that arbitrary combinations of CTL properties can be expressed. 
...
The definitions follow directly from the satisfiability relation for CTL properties in section~\ref{sec:computation_tree_logic}. 
...
\textit{CTL semantics} for path dependent operators `until' and `global' are defined in terms of fixed-points. 
These fixed-points are defined over the partially ordered set of ranking functions $\langle \Sigma \rightarrow \mathbb{N}, \sqsubseteq \rangle$. 
...
The \textit{CTL semantics} for universal and existential `until' properties are defined as least fixed-point of the abstract transformers
\begin{align*}
...
Furthermore, by starting from $0$ at states that satisfy $\Phi_2$ and incrementing while backtracking, 
we construct a ranking function sucht that the value assigned to each state is an 
...
iterative computation for `until' properties works for universal and existential quantifiers. 
...
The initial state is added to the ranking function in the last iteration because there exists one edge that leads to a state satisfying the property.  
...
The fixed-point iteration starts with the ranking function $\tau_\Phi$ of the inner property $\Phi$.
...
Definition~\ref{def:ctl_semantics_next} defines the CTL `next' operator. The `next' operator is path dependent but does not
require fixed-point iterations. A state satisfies $\forall\bigcirc\Phi$ if all its successors satisfy the property $\Phi$,
correspondingly $\exists\bigcirc\Phi$ is satisfied if at least one successor satisfies the property $\Phi$. 
This corresonds to the definition of the $\widetilde{pre}$ and $pre$ relations. Zero is assigned to each state
that satisfies the property to construct a valid ranking function.
...
The first one joins leafs (by forming the smallest upper bound) with $\sqsubseteq_{\mathcal{F}}$ the latter with $\preceq_{\mathcal{F}}$. 
...
Both combine defined leafs using the smallest upper bound according to $\preceq_{\mathcal{F}}$. 
...
\subsection{Reset}
The $\mathtt{RESET}\llbracket bexp \rrbracket \colon \mathcal{T}_\mathtt{NIL} \rightarrow \mathcal{T}_\mathtt{NIL}$ operator can be used
to reset the value of the ranking function to zero for all partitons that satisfy the boolean expression. This operator has the same implementation
as $\mathtt{FILTER\_UNDER}\llbracket bexp \rrbracket$ but replaces leafs that do not satisfy the property with the constant function
$0$. Furthermore it uses undreapproximation to guarantee that only those partitions are reset that actually satisfy the boolean expression. 
...
halting problem. Therefore, we approximation $\tau_\Phi$ by using
...
initial program label $\tau^{\sharp}_\Phi(t_{init})$ is defined over all partitions i.e.\ all leafs of the decision tree are defined.\\
...
\begin{tcolorbox}
    ...
    in section~\ref{sec:decision_tree_abstract_domain}.
\end{tcolorbox}
...
we ensure that no $\mathtt{NIL}$ leafs are introduced when forming the intersection.\\
...
The definitons for the remaining path dependent operators all depend on these two functions.
...
\end{align*}
starting from the 
totally undefind decision tree $\bot_\mathcal{T}$ (see definition~\ref{def:abstract_until_semantics}).
