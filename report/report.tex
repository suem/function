...
\newcommand{\nextime}{\ensuremath{\bigcirc}}
\newcommand{\until}{\ensuremath{\mathrel{\mathsf{U}}}}

\newcommand{\cmark}{\textcolor{green!75!black}{\ding{51}$^{\ }$}} % check mark
\newcommand{\nmark}{-$^{\ }$} % null mark
\newcommand{\smark}{\textcolor{green!75!black}{\ding{51}$^*$}} % starred check mark
...
\begin{abstract}
    ...
    Furthermore, I am grateful to Prof. Peter M\"uller for giving me the opportunity to work on this project.
\end{abstract}
...
We assume $x \geq 0$ at the start of the program. 
\textsf{SAMPLE} decrements $x$ until it reaches the value $0$ if either $y = 1$ or if a random non-deterministic coin toss succeeds.
Otherwise the program sets $x$ to $-1$.
Because of non-determinism, this program has two possible execution traces, one where the if-block is entered and one
where it is not. Consider the CTL property $y = 1 \Rightarrow \forall(x \geq 0 \ U \ x = 0)$. 
Informally, the property states that if initially $y = 1$, i.e., the
if-block is entered, then $x \geq 0$ holds among all possible execution traces until a state is reached where $x=0$ holds
(a formal definition of CTL can be found in Section~\ref{sec:computation_tree_logic}). Due non-determinism only one of the
two program traces modifies $x$ as stated in the property. 
This fact can be expressed with the CTL property $\exists(x \geq 0 \ U \ x = 0)$. 
This states that there exists at least one execution trace
where $x \geq 0$ holds until a state is reached where $x = 0$ holds. This example demonstrates the difference between universal and existential 
CTL properties and it shows the expressiveness of CTL. Note that there are temporal logic systems like CTL$^{*}$
that are strictly more expressive than CTL~\cite{baier2008principles}. 
We chose CTL because it is a natural extension of 
...
$\exists(x \geq 0 \ U \ x = 0)$ from the previous example. This property is a liveness property that states that $x = 0$ holds eventually for at least
...
If the initial value of $x$ is greater than $0$ then the goal of the liveness property is reached in exactly $2x + 1$ program execution steps 
($2x$ steps inside the while loop plus one step to enter the if-block).
...
is satisfied if $x < 0$. In this case it is clear that the property is not satisfied if $x < 0$, however, the analysis is not able to 
prove or disprove all CTL properties in general.\\

Deciding if a CTL property is satisfied is in general undecidable. We achieve a decidable approximating of counting functions by performing
...
By expressing the possible transitions between states in terms of a relation, 
...
This condition can be relaxed for existential \textsf{until} properties by using the $pre$ relation instead. 
...
are actually undefined in the concrete counting function. Partitions that are undefined because of this uncertainty are marked with a 
...
This can yield more precise results in some cases (see Example 5 in the next subsection).\\
...
\caption{Table of test cases. Lists the number, program name, CTL property to be analyzed, result of the analysis and runtime.
    Test cases with a \smark use ordinal-valued counting functions. Program names marked with ${}^*$ are a slightly adjusted version of the original.
}
...
The three failed test cases could not be verified due to imprecision of the decision tree abstract domain. 
...
\caption{Table with results of test cases for FuncTion, T2 and Ultimate LTL Automizer. 
    ...
    Program names marked with ${}^*$ are a slightly adjusted version of the original.
}
...
Furthermore, only FuncTion is able to analyze existential \textsf{until} properties.
\\
...
for the domain of piecewise-defined linear functions.\\
...
\includepdf[clip]{declaration.pdf}
